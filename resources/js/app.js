...
$(document).ready(function() {
    ...
    $(".street-field, .number-field, .city-field, .state-field").on(
        "focusout",
        function() {
...
$(document).ready(function() {
    ...
    if ($("#home-where-input").length) {
        // verifico se l'id è definito, cioè se sono sulla pagina home
        // ripulisco il campo di ricerca della località
        $("#home-where-input").val("");
    }

    $("#home-where-input").keyup(function() {
        console.log("ajax call to tomtom...");
        var apiKey = "BG5ffg9ACWQBPZZHShDaXxBnheo0bD36";

        place = $("#home-where-input")
            .val()
            .trim();
